...
export default function TaskManagerTask (props) {
  const observableProject = <ObserveString ob={stores.projectStore.getProjectOb(props.task.project)} t={x => x && x.name} />
  const observableDisplayName = <ObserveString ob={stores.taskDisplayNameStore.getTaskDisplayNameOb(props.task.url)} />
  ...
  return <tbody>
    <tr>
      ...
      {!this.state.editing
        ...
        : <td>
          <input value={props.newName} placeholder={props.task.name} onChange={props.onNewNameChanged} xonChange={e => this.setState({newName: e.target.value})} />
